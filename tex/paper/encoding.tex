% ── encoding.tex · §2 ──────────────────
\section{Encoding \pp{} in \lp}\label{sec:encoding}

\subsection{The Predicate Prover}\label{sec:pp-syntax}

The syntax for the \emph{predicates} and \emph{expressions} of \pp{} is
given as follows:
\begin{align*}
  E, F &\Coloneqq x \mid n \mid E \mapsto F \mid f(E_1,\ldots,E_k)
         \mid E + F \mid E - F \mid -E \\
  P, Q &\Coloneqq E \mid E = F \mid E \leq F
         \mid E_1,\ldots,E_k \in F
         \mid \lnot P \mid P \land Q \mid P \lor Q \mid P \Rightarrow Q
         \mid P \Leftrightarrow Q
         \mid \mathcal{Q}\,\vec{x} \cdot P
\end{align*}
where $x$ ranges over identifiers, $n$ over numerals, $f$ over function
symbols, $\mapsto$ is B's pairing, membership relates a tuple of
expressions to an expression, and an expression standing alone is an
atom (this is how propositional variables appear).
%
A quantifier $\mathcal{Q} \in \{\forall, \exists, \Dia, \Hrt\}$ binds a
list of variables, and the list forms a single \emph{compound} binder:
$\forall (x,y) \cdot P$ is distinct from $\forall x \cdot \forall y
\cdot P$, and dedicated rules (\rn{ALL1--4}, \rn{XST1--4}) regroup the
latter into the former during proof search.
%
The quantifiers $\Dia$ and $\Hrt$ are universal quantifiers idiosyncratic
to \pp's normalisation procedure; semantically both are $\forall$ and differ
only in the role they play during proof search.

Although B is a set-theoretic formalism, \pp{} itself proves formulas of
first-order logic with linear arithmetic: a pre-processing pass reduces
the set theory of a proof obligation to first-order formulas over
membership before \pp{} sees it.
%
The set-theoretic operators that survive this reduction (intervals,
relational operators, \ldots) reach \pp{} as uninterpreted function
symbols: \pp{} has no set-theory rules, and such terms are only ever
copied, compared for equality, or instantiated.

\pp{} proves \emph{sequents} $H \seq P$, where $H$ is a list of
hypothesis predicates and $P$ a goal predicate.
%
Alongside sequents, its proofs manipulate a second judgment form: during
normalisation a rule does not close a sequent but transforms a formula
and passes on its \emph{result}, the conjunction of the formula's
surviving conjuncts.
%
We write $P \yields Q$ for ``$P$ normalises with result $Q$''.
%
\pp's inference rules are specified in the annex of its internal
specification~\cite{pp_spec}: roughly \numRulesSpec{} rules, grouped by
families — propositional (\rn{AND}, \rn{OR}, \rn{IMP}, \rn{EQV},
\rn{NOT}), hypothesis discharge (\rn{AXM1--9}), quantifiers
(\rn{ALL1--9}, \rn{XST1--8}), equality and the equality prover
(\rn{EQL}, \rn{EAXM}, \rn{EQC}, \rn{ECTR}, \ldots), booleans
(\rn{BOOL}), linear arithmetic (\rn{AR1--13} plus the solver terminals
\rn{ARITH} and \rn{EGALITE}), and normalisation and instantiation
(\rn{NRM1--30}, \rn{INS}).
%
A rule in sequent form reduces the current goal to zero, one, or two
premises, possibly under computational side-conditions
(``$P$ occurs in the list of hypotheses $H$'',
 ``$a$ and $b$ are integer literals such that $a + b < 0$'', etc.).
%
For a rule in result form, the specification (\S 8.13) prescribes one of
three schemas: its result is the constant true (the formula is
discharged), a single transformed formula, or the combination of the
results of the rule's two children.

\pp{} can be instrumented to record a compact \trace{} of the rules it
applied; a companion utility (\textsc{replay}) re-runs the proof and
expands the trace into a \replay{} that annotates every step with the
formula it was applied to.
%
Each replay line carries a bracketed rule name and that formula;
result-form applications bear the suffix \texttt{\_1}, and a rule
argument (a hypothesis subscript, a binder count, a predicate) may
follow the name in parentheses.
%
The main proof is recorded prefix-style (a rule precedes its
subproofs), with two irregularities: the chain of result-form steps
feeding a branching quantifier (\rn{ALL7}, \rn{XST8}) is emitted
\emph{before} the branch rule itself, and normalisation markers
(\texttt{FIN}, \texttt{STOP\_NORM}, \texttt{NRM}) that carry no proof
content are interleaved throughout.
%
The annotation of the first non-marker line is the overall goal.
%
\autoref{fig:replay} shows a complete replay from our benchmark suite
next to the derivation it denotes.

% --- replay/derivation figure ----------------------------------------
\begin{figure}[t]
  \centering
  \begin{minipage}[c]{0.40\linewidth}
\begin{codePP}
[ALL2] <not(!x.!y.not(x = x))>
[ALL6] <not(!(x,y).not(x = x))>
[STOP_1] <FAUX>
[EVR2_1] <not(x = x)>
[ALL7(1)] <!(x,y).not(x = x) => FAUX>
[FIN(FAUX)] <$\ldots$>
[NRM1] <forall(x,y).FAUX => FAUX>
[STOP_NORM] <FAUX => FAUX>
[NRM] <forall(x,y).FAUX => FAUX>
[FIN(FAUX => FAUX)] <$\ldots$>
[AXM7] <FAUX => FAUX>
\end{codePP}
  \end{minipage}\hfill
  \begin{minipage}[c]{0.57\linewidth}
    \footnotesize
    \begin{mathpar}
      \inferrule*[Right=\rn{ALL2}]{
        \inferrule*[Right=\rn{ALL6}]{
          \inferrule*[Right=\rn{ALL7}]{
            \inferrule*[Right=\rn{EVR2\_1}]{
              \inferrule*[Right=\rn{STOP\_1}]{ }{\bot \yields \bot}}
            {\lnot(x = x) \yields \bot}
            \\
            \inferrule*[Right=\rn{NRM1}]{
              \inferrule*[Right=\rn{AXM7}]{ }{\seq \bot \Rightarrow \bot}}
            {\seq (\Dia (x,y) \cdot \bot) \Rightarrow \bot}}
          {\seq (\forall (x,y) \cdot \lnot(x = x)) \Rightarrow \bot}}
        {\seq \lnot \forall (x,y) \cdot \lnot(x = x)}}
      {\seq \lnot \forall x \cdot \forall y \cdot \lnot(x = x)}
    \end{mathpar}
  \end{minipage}
  \caption{Benchmark \texttt{og/22}: a replay (left; \texttt{FIN}
    annotations elided) and the derivation it denotes (right), reading
    \texttt{!} as $\forall$, \texttt{forall} as $\Dia$, \texttt{FAUX}
    as $\bot$, and \texttt{=>} as $\Rightarrow$.
    Each non-marker line is one rule application, in prefix order
    except for the result chain (\rn{STOP\_1}, \rn{EVR2\_1}), which is
    emitted before the \rn{ALL7} that consumes it.
    \rn{ALL2} regroups the nested binders into a compound one,
    \rn{ALL6} unfolds the negation, and \rn{ALL7} replaces the body of
    the universal antecedent by the chain's result before \rn{NRM1} and
    \rn{AXM7} close the proof.}
  \label{fig:replay}
\end{figure}
% ---------------------------------------------------------------------

\subsection{\lp}\label{sec:lp}

\lp~\cite{hondet:hal-02981561} is a proof assistant for the
\lpcmr~\cite{CousineauDowek}, the calculus underlying the \dedukti{}
logical framework~\cite{assaf2023dedukti}.
%
Its terms are those of a dependently-typed $\lambda$-calculus,
\[
  t, u \Coloneqq \TYPE \mid \KIND \mid x \mid c
        \mid t\,u \mid \lambda (x : t),\, u \mid \Pi (x : t),\, u
\]
where $c$ ranges over declared constants; conversion is generated by
$\beta$ together with user-declared rewrite rules, and types are
identified up to conversion.
%
A development is a sequence of declarations.
%
\lpkw{symbol} introduces a typed constant, with explicit
$(x : A)$ and implicit $[x : A]$ parameters and an optional definition;
\lpkw{rule} $\ell ↪ r$ adds a rewrite rule over pattern variables
\texttt{\$x}, checked for subject reduction at declaration time;
\lpkw{inductive} generates constructors and a recursor.
%
Modifiers refine a symbol's status: \lpmod{constant} symbols admit no
rewrite rule at their head, \lpmod{injective} symbols may be inverted
during unification, and the definition of an \lpmod{opaque} symbol is
sealed once type-checked.
%
A definition may be given as a proof script (\lpkw{begin} \ldots{}
\lpkw{end}) whose tactics --- \lpkw{assume}, \lpkw{refine},
\lpkw{rewrite}, \lpkw{have}, \lpkw{induction} --- ultimately
elaborate to a proof term that the kernel re-checks, so the tactic layer
adds nothing to the trusted base.

We build on \lp's standard library~\cite{Blanqui2021}: object-level types are
encoded in a Tarski-style universe $\Setu$ with $\El : \Setu \to \TYPE$
mapping each set code to its type of inhabitants, and a type $\Prop$ of
propositions with $\Prf : \Prop \to \TYPE$ mapping each proposition
to its type of proofs.
%
Connectives and quantifiers are the standard impredicative encodings;
on top of these we assume the library's classical axioms — excluded
middle and propositional extensionality.

\subsection{Encoding \pp's Predicates and Expressions}\label{sec:enc-formulas}

The encoding is shallow.
%
All B expressions inhabit a single domain: a set code $\iota$, so an
expression is a term of type $\El\,\iota$ and a predicate is a term of
type $\Prop$.
%
The library's code $o : \Setu$, with $\El\,o ↪ \Prop$, lets predicates
also occur where set elements are expected; the conjunction lists below
rely on this.
%
The kernel file \texttt{B.lp} declares the domain constants —
$\mapsto$ (pairing), $\epsilon$ (membership), $\mathsf{BOOL}$ with
$\mathsf{BTRUE}$ and $\mathsf{BFALSE}$, B's functional application, and
the arithmetic signature $\mathbf{0}, \mathbf{1}, +, -, \leq$ — together
with their axioms: injectivity and congruence of pairing, set
extensionality, the two-element axioms for $\mathsf{BOOL}$, and for
arithmetic the theory of a discretely ordered abelian group
($+$ commutative and associative with identity $\mathbf{0}$ and inverse
$-$; $\leq$ a partial order, invariant under translation; and the
discreteness axioms $\mathbf{0} \leq \mathbf{1}$,
$\lnot(\mathbf{1} \leq \mathbf{0})$ and
$\lnot(x \leq \mathbf{0}) \Rightarrow \mathbf{1} \leq x$).
%
This is just enough to \emph{derive} \pp's arithmetic rules; it is not a
formalisation of $\mathbb{Z}$.
%
Numerals are decimal literals of the library's $\mathbb{N}$, coerced
into the domain by an embedding that \lp{} inserts automatically.
%
Every other operator — the set-theoretic surface of
\autoref{sec:pp-syntax} included — is an uninterpreted constant.

\paragraph{$n$-ary conjunction.}
\pp{} treats $\land$ as left-associative and its rules address a
conjunction through its \emph{last} conjunct and the remainder; worse,
when a normalisation step discharges a conjunct, \pp{} silently drops it
from every formula it subsequently records.
%
Encoding $\land$ binarily would therefore force proofs to rewrite
associativity and unit collapses at nearly every step.
%
Instead, a conjunction is a \emph{snoc list}: an inductive type
$\mathbb{L}^{*}$ with empty list $∎$ and right append $∷$, and an
injective operator $\bwand : \mathbb{L}^{*}\,o \to \Prop$ with the
rewrite rules
$\bwand(∎ ∷ P) ↪ P$ and
$\bwand(ps ∷ \top) ↪ \bwand ps$
(a singleton conjunction \emph{is} its conjunct; discharged conjuncts
erase).
%
Its structural interface — introduction, and elimination of the last
conjunct and of the remainder — is axiomatised.
%
\pp's conjunction rules then become single structural matches; for
example (\texttt{lp/rules/Conj.lp}):
\begin{codeLP}
opaque symbol AND2 [ps : $\mathbb{L}^{*}$ o] [P : Prop]
  : π ($\bigwedge$ ps ⇒ ¬ P) → π (¬ $\bigwedge$ (ps ∷ P)) ≔
begin
  assume ps P h1 hc;
  refine h1 ($\bigwedge$_init hc) ($\bigwedge$_last hc)
end;
\end{codeLP}

\paragraph{Compound binders.}
\pp{} distinguishes the compound binder $\forall (x,y) \cdot P$ from the
nested $\forall x \cdot \forall y \cdot P$ (\autoref{sec:pp-syntax}),
and its quantifier rules manipulate a compound binder as a unit.
%
We mirror this with an inductive dependent type $\Tuple : ℕ → \TYPE$
with projections $\prj [n] : \Tuple\,n → \El\,\iota$, and quantifiers
$\allI, \xstI : (\Tuple\,n \to \Prop) \to \Prop$
whose introduction and elimination principles are axiomatised bridges to
the $\Pi$-types of \lp{} (e.g.\
$(\Pi v{:}\,\Tuple\,n,\ \Prf\,(P\,v)) \to \Prf\,(\allI v, P\,v)$ and its
converse); $\Dia$ and $\Hrt$ are defined aliases of $\allI$ with bridges
of their own.
%
The regrouping rules (\rn{ALL1--4}, \rn{XST1--4}) need no \lp{}
counterpart: our translation regroups binders itself while building the
goal, so the \lp{} goal always carries one compound binder and these
rules are consumed without emitting anything (\rn{ALL2} in
\autoref{fig:replay} is an instance).
%
% we probably don't need to make this point but it's useful to have:
% -------------------------------------------------------------------
% No rewrite rule unfolds $\allI$ to nested $\forall$: if the quantifier
% reduced away, the surface form would vanish from goals before any rule
% lemma could address it, and inferring the
% implicit predicate $P$ at rule applications would require inverting the
% reduction, a higher-order unification problem \lp{} does not attempt.
%

\subsection{Encoding Rules}\label{sec:enc-rules}

Each \pp{} rule is a \lp{} symbol whose type states the rule and whose
body \emph{proves} it.
%
The encoding of a rule's statement is positional: the conclusion sequent
$H \seq P$ appears as $\Prf\,P$; a premise that extends the hypotheses,
$H, P \seq Q$, appears as the function type $\Prf\,P \to \Prf\,Q$; and a
side-condition ``$P$ occurs in $H$'' becomes an explicit argument of
type $\Prf\,P$, which our translation tool supplies by looking the hypothesis
up in scope.
%
For example, \rn{IMP4} (``to prove $P \Rightarrow Q$, assume $P$ and
prove $Q$'') and the discharge rule \rn{AXM1} (``$\lnot P$ occurs in
$H$, so $P \Rightarrow Q$ holds'') have the following type signatures:
\[
  \rn{IMP4} : (\Prf\,P \to \Prf\,Q) \to \Prf\,(P \Rightarrow Q)
  \qquad
  \rn{AXM1} : \Prf\,(\lnot P) \to \Prf\,(P \Rightarrow Q).
\]

\paragraph{Result chains.}
To encode the rules' result forms we package the judgment
$t \yields \bwand ts$ as a record: a value of type $\Res\,t$ pairs the
list $ts$ of surviving conjuncts with a proof of
$\Prf\,(t = \mathsf{conj}\,ts)$, where $\mathsf{conj}$ folds a list
back into a formula.
%
The three specification schemas (\autoref{sec:pp-syntax}) map directly
onto its constructors: a discharged formula is the empty list with a
proof of $t = \top$; a transformation reuses a proved equality law (for
example, \rn{NOT2}'s result form is
$\mathsf{mk_1}\,(\restm\,r)\,(\mathsf{eq\_trans}\ \texttt{¬¬-law}\
(\mathsf{res\_eq}\,r))$); and a combination concatenates the two
children's lists, justified by a concatenation law for $\mathsf{conj}$.
%
The $\top$-erasure rewrite rules on $\bwand$ and on list concatenation
make the combined result reduce to exactly the conjuncts \pp{} records —
discharged conjuncts disappear in any position, mirroring \pp.
%
A branching quantifier consumes a chain of such steps.
%
\rn{ALL7} normalises the body of a universal antecedent pointwise and
continues under the normalised form:
\begin{codeLP}
opaque symbol ALL7 [n] [P : Tuple n → Prop] [Q : Prop]
  (ρ : Π v : Tuple n, Res (P v))
  : π ((`$\diamondsuit$ v, res_tm (ρ v)) ⇒ Q) → π ((`!! v, P v) ⇒ Q) ≔ $\ldots$
\end{codeLP}
where the proof transports each instance along the chain's equality
($\mathsf{res\_eq}$).
%
Chains are emitted as explicit terms rather than tactic steps; for the
replay of \autoref{fig:replay} the whole emitted proof is
\begin{codeLP}
refine ALL7 (λ v, EVR2_1 STOP_1) _;  refine NRM1 _;  refine AXM7
\end{codeLP}
— \rn{ALL2} emits nothing (\autoref{sec:enc-formulas}), and neither does
\rn{ALL6}, since $\lnot P$ and $P \Rightarrow \bot$ are definitionally
equal in the encoding.

\paragraph{Solver side-conditions.}
Some rules are licensed not by a structural premise but by a verdict of
\pp's arithmetic solver or equality prover: \rn{AR4} requires
$(E+F) > \mathbf{0}$ for terms \pp{} already believes positive, and the
\rn{ARITH} terminal closes a goal because the accumulated
$\leq$-hypotheses are contradictory.
%
The corresponding lemmas take these facts as explicit $\Prf$-arguments,
and \tool{} \emph{generates} the witnessing proof at emission time.
%
For \rn{ARITH} the witness is a nonnegative (Farkas-style) combination
of the $\leq \mathbf{0}$ hypotheses: with
$h_1 : \Prf\,(x \leq \mathbf{0})$ and
$h_2 : \Prf\,(\mathbf{1} - (x + x) \leq \mathbf{0})$ in scope, the
emitted refutation is
\begin{codeLP}
one_not_leq_zero (leq_subst_l e (add_leq_zero _ _ (add_leq_zero _ _ h2 h1) h1))
\end{codeLP}
which sums the hypotheses to $\mathbf{1} \leq \mathbf{0}$; the equality
$e : \Prf\,(\mathbf{1} = (\mathbf{1}-(x+x))+x+x)$ justifying the sum is
itself generated from the group axioms.
%
The same machinery produces equality-transport chains for the equality
prover, and permutation proofs wherever \pp{} recorded a sum in a
different association order than the lemma needs.

\paragraph{The trusted base.}
A reconstructed proof type-checks against exactly: the \lp{} kernel, the
standard library with its classical axioms, the B domain axioms of
\autoref{sec:enc-formulas}, the structural interfaces of $\bwand$ and
$\Res$, and the quantifier bridges.
%
Everything else, the rule library included, is a theorem.
%
Nor can the translator smuggle in an assumption: its term grammar has no
constructor for an oracle, so a side-condition for which no witnessing
proof can be generated is a hard failure rather than a silent axiom.
