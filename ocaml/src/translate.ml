...
and chain_term ctx node : L.term =
  ...
  | P.Apply { rule; anno; children = [c]; _ }
    when base rule = "NRM2" ->
    (* NRM2_1 (r : Res ((P ⇒ ♢v, Q v) ⇒ S)) : Res ((♢v, P ⇒ Q v) ⇒ S).
       PP's chain NRM2 is the spec pullout: the v-free antecedent P moves out of
       the ♢-body.  The two antecedents are *equivalent* (P free of v), so no
       proof of P is needed — the child already carries `P ⇒ ♢v, Q v`. *)
    (match goal_of_anno anno with
     | Some (Binary (Imp, Bind (_, _, Binary (Imp, _, _)), _)) ->
       L.App (L.Name (chain_emit_name rule), [chain_term ctx c])
     | _ ->
       Errors.fail "E_EMIT"
